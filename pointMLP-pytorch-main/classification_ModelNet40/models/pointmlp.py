...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        #
        x2 = torch.from_numpy(core).cuda()
        xyz = x.permute(0, 2, 1)  # [4, 1024, 3]
        batch_size, _, _ = x.size()
        ...
        x = x.permute(1, 0)
        x2 = x2.permute(1, 0)
        # output = torch.cosine_similarity(x, x2).cuda()
        output = torch.cosine_similarity(x, x2).cuda()
        ...
        out = torch.tensor(out).to(torch.float32).cuda()
